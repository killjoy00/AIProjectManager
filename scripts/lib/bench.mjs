// The idea bench: one rolling issue the Sunday sweep appends to, and the weekly brief harvests.
//
// Both halves now run in the same Sunday run: the sweep proposes 1–3, the brief surfaces at most
// three from the whole accumulated bench. The split still matters — proposing is cheap and the
// bench keeps everything, while publishing is restrained because that is where attention is spent.
//
// The midweek check-ins deliberately do not touch this. Idea generation on a run that has not read
// any source is noise, and a bench that grows on every run trains the owner to stop reading it.
...
// feedback taught, once, beats re-deriving it from forty comments every run — it compounds
...
const BENCH_BODY = [
  "<!-- apm:system -->",
  "Ideas the sweep thought might suit this portfolio. **Nothing here is a commitment.**",
  "",
  "One rolling issue rather than an issue per idea — an idea nobody asked for should not cost a",
  "triage decision. The weekly brief surfaces at most three; the rest sit here until you want them",
  "or they age out of relevance.",
  "",
  ...
  "",
  "Maintained by the sweep from your reactions. **Edit it freely** — if it has drawn the",
...
// same thing every week, and so the owner's rejections are visible to it.
...
export async function postIdeas(ideas) {
  ...
  const clean = ideas
    ...
    .slice(0, 3)
    .map((i) => ({
      // Same reason as post-sweep: the marker and footer are this file's to add, and a model
  ...
  const body = [
    ...
    "<sub>Generated by the portfolio sweep. Not a commitment and not a task — ignore freely.</sub>"
  ].join("\n");
