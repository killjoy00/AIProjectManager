...
// Markers let us tell our own machine-written comments apart from human ones. All of them live
// here rather than beside the code that writes each one, because two places need the *complete*
// set — stripAgentStamps below, and collect-context.mjs when it decides which comments are
// third-party. A marker defined elsewhere and forgotten in those lists is how a bot comment ends
// up classified as untrusted stranger text.
...
export const HANDOFF_MARKER = "<!-- apm:handoff -->";

// Every marker this system writes into a comment body. Used to recognise our own output.
export const MACHINE_MARKERS = [
  TRIAGE_MARKER, BRIEF_MARKER, SYSTEM_MARKER, IDEA_MARKER, SPINOFF_MARKER, HANDOFF_MARKER
];

export const isMachineComment = (body) =>
  MACHINE_MARKERS.some((m) => (body || "").includes(m));
...
// The model is told to write the comment body only — the marker, the dated header and the
// footer are added here, so the posting script owns them and they can't be forged or omitted.
// The model writes them anyway sometimes, having seen them on every previous comment it read,
// and the result is a comment with its header printed twice. Rather than sharpen the prompt and
// hope, strip them: the stamping code is the only thing allowed to produce them.
// The old header names stay in this list on purpose. The model reads past comments from the
// context file and copies whatever format it sees there, so it will keep emitting "Nightly triage"
// headers for as long as any remain in the threads. Dropping the old names when the workflow was
// renamed would have quietly reintroduced the duplicated-header bug.
const STAMP_PATTERNS = [
  /<!--\s*apm:(?:triage|brief|idea|system|spinoff|handoff|status:start|status:end|learning:start|learning:end)\s*-->/g,
  /^\*\*(?:Nightly triage|Portfolio sweep|Mid-week check-in|Weekly brief|Handoff packet)\s+—\s+\d{4}-\d{2}-\d{2}\*\*\s*$/gm,
  /^<sub>Automated\. Agent is comment-only;[^<]*<\/sub>\s*$/gm,
  /^<sub>Generated by (?:nightly triage|the portfolio sweep)\.[^<]*<\/sub>\s*$/gm,
  /^<sub>The manager cannot build this[^<]*<\/sub>\s*$/gm
];

export function stripAgentStamps(body) {
  let out = String(body || "");
  for (const re of STAMP_PATTERNS) out = out.replace(re, "");
  // Collapse the blank lines the removals leave behind, so the comment doesn't open with a gap.
  return out.replace(/\n{3,}/g, "\n\n").trim();
}
